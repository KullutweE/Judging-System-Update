...
const renderUser = (doc) => {
  ...
  btnDelete.addEventListener("click", () => {
    db.collection("Judges")
      ...
      .then(() => {
        let judgeCount=0
        db.collection("Judges").get().then(function(querySnapshot) {
...
addModalForm.addEventListener("submit", (e) => {
  e.preventDefault();
  db.collection("Judges").get().then((snapshot)=>{
    let judgeCount=0;
    let maxNum=0;
    
   snapshot.docs.forEach(each=>{
      if(each.data().judgeNumber>maxNum){
        maxNum=each.data().judgeNumber;
      }
      judgeCount++;  
...
const tableDisqualified = document.querySelector(".table-disqualified");
...
//Disqualified list Rendering
let disqualifiedNum = 0;
const disqualifiedListRendering = (doc) => {
  const tr = `
    <tr data-idapp='${doc.id}'>
      <td>${++disqualifiedNum}</td>
      <td>${doc.data().firstName}</td>
      <td>${doc.data().lastName}</td>
  ...
  `;
  tableDisqualified.insertAdjacentHTML("beforeend", tr);
};
...
const acceptedListRendering = (doc) => {
  const tr = `
    <tr data-idapp='${doc.id}'>
      ...
      <td>
      <button class="btn btn-disqualify">Disqualify</button>
      </td>
    </tr>
  `;
  if (doc.data().Gender === "female")
  ...
  const btnDisqualify = document.querySelector(
    `[data-idapp='${doc.id}'] .btn-disqualify`
  );

  btnDisqualify.addEventListener("click", () => {
    let tr = document.querySelector(`[data-idapp='${doc.id}']`);
    let tbody = tr.parentElement;
    tbody.removeChild(tr);
    db.collection("Disqualified")
      .doc(doc.id)
      .set(doc.data())
      .then(() => {
        console.log("Document written to disqualified collection !");
      });
...
const renderUserapp = (doc) => {
  ...
  ///Data for function below
  //first: getting count of contestants already present so that
  //we assign unique id++ every time we add new user
  ...
  btnAccept.addEventListener("click", () => {
    let prevCountObject;
    let prevCount;
    let acceptedApplicantInfo = doc.data();
    db.collection("ContestantsNumber").doc("counter").get().then(docum=>{
      prevCountObject=docum.data();
      if (acceptedApplicantInfo.Gender === "male") {
        db.collection("ContestantsNumber")
          .doc("counter")
          .set({
            ...prevCountObject,
            male: prevCountObject.male + 1,
          });
        prevCount = prevCountObject.male;
      } else {
        db.collection("ContestantsNumber")
          .doc("counter")
          .set({
            ...prevCountObject,
            female: prevCountObject.female + 1,
          });
  
        prevCount = prevCountObject.female;
      }
      db.collection("AcceptedApplications")
      .doc(doc.id)
      .set({ ...acceptedApplicantInfo, ContestantNo: prevCount + 1 })
      .then(() => {
        console.log("Document successfully written!");
        db.collection("Applications")
          .doc(`${doc.id}`)
          ...
          .then(() => {
            console.log("Document successfully deleted!");
            let params={
              to_name : doc.data().firstName,
              applicant_email : doc.data().email,
              message : "Congratulations! you are selected."
              
            }
            console.log("sending mail");
            emailjs.send("service_ydqc2pm","applicant_form",params).then(()=>{
              console.log("success");
            }).catch((err)=>{
              console.log(err);
            });
    ...
    //in the code above, we find out whether the accepted application is of
    //male or female.We also update our counter accordingly
    //Now we will write this document in acceptedApplications collection and
  ...
  snapshot.docChanges().forEach((change) => {
    ...
    if (change.type === "modified") {
      ...
      if (change.doc.data().Gender === "male") {
        ...
        tableUsersappFemale.removeChild(tbody);
        renderUserapp(change.doc);
      }
    }
  });
...
//realtime update of rejected users
db.collection("RejectedApplications").onSnapshot((snapshot) => {
  snapshot.docChanges().forEach((change) => {
    if (change.type === "added") {
      rejectedListRendering(change.doc);
    }
    if (change.type === "removed") {
      let tr = document.querySelector(`[data-idapp='${change.doc.id}']`);
      let tbody = tr.parentElement;
      ...
      if (change.doc.data().Gender === "male")
        tableRejectedMale.removeChild(tbody);
      else tableRejectedFemale.removeChild(tbody);
    }
    if (change.type === "modified") {
      let tr = document.querySelector(`[data-idapp='${change.doc.id}']`);
      let tbody = tr.parentElement;
      if (change.doc.data().Gender === "male") {
        ...
        tableRejectedFemale.removeChild(tbody);
        rejectedListRendering(change.doc);
      }
    }
  });
...
//realtime update of accepted users
// db.collection("AcceptedApplications").onSnapshot((snapshot) => {
//   snapshot.docChanges().forEach((change) => {
//     if (change.type === "added") {
//       acceptedListRendering(change.doc);
//     }
//     if (change.type === "removed") {
//       let tr = document.querySelector(`[data-idapp='${change.doc.id}']`);
//       let tbody = tr.parentElement;
...
//       //
//       //       if (change.doc.data().Gender === "male") {
//       //         tableAcceptedMale.removeChild(tbody);
//       //       } else {
//       //         tableAcceptedFemale.removeChild(tbody);
//       //       }
//     }
//     if (change.type === "modified") {
//       let tr = document.querySelector(`[data-idapp='${change.doc.id}']`);
...
//get disQualified contestants
db.collection("Disqualified")
  .get()
  .then((querySnapshot) => {
    querySnapshot.forEach((change) => {
      if (change.type === "added") {
        disqualifiedListRendering(change.doc);
      }
    });
  });
//Realtime update of disqualified contestants
db.collection("Disqualified").onSnapshot((snapshot) => {
  snapshot.docChanges().forEach((change) => {
    disqualifiedListRendering(change.doc);
  });
});
